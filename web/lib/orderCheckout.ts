...
/** Envío a domicilio activo en este producto (modos de la publicación + ciudades tarifadas). */
...
export function generateShippingOptions(product: Product): ShippingOption[] {
  ...
  if (product.tipoDeEntregaPermitida?.recogidaEnPersona) {
    options.push({
      ...
      description: product.ciudadRecogidaEnPersona
        ? `En ${product.ciudadRecogidaEnPersona}`
...
/** Objeto limpio para Firestore (sin claves en 0). */
