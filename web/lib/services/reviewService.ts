...
/** Solo las categorías explícitamente calificadas por el comprador. */
export type PartialReviewCategories = Partial<ReviewCategories>

/** Nota general coherente: promedio de categorías detalladas si existen, si no la nota global. */
export function resolveReviewRating(generalRating: number, categories?: PartialReviewCategories): number {
  const values: number[] = []
...
function mapDocToSellerReview(id: string, data: Record<string, unknown>): SellerReview {
  return {
    ...
    sellerResponseDate: typeof data.sellerResponseDate === "string" ? data.sellerResponseDate : undefined,
  }
}
...
export function computeSellerReviewStats(reviews: SellerReview[]): SellerReviewStats {
  ...
  let sum = 0
  const catSum: Record<ReviewCategoryKey, number> = { comunicacion: 0, descripcion: 0, envio: 0, calidad: 0 }
  const catCount: Record<ReviewCategoryKey, number> = { comunicacion: 0, descripcion: 0, envio: 0, calidad: 0 }
