...
export type AddressInput = Omit<SavedAddress, "id" | "isVerified" | "lastUsedAt" | "createdAt" | "updatedAt">
...
export async function updateAddress(userId: string, addressId: string, input: Partial<AddressInput>): Promise<void> {
  const ref = doc(db, "users", userId, "addresses", addressId)
  const snap = await getDoc(ref)
