...
function mapSnapToBuyerOrder(docSnap: { id: string; data: () => Record<string, unknown> }): BuyerOrder {
  ...
  return {
    ...
    sizeQuantities:
      sq && typeof sq === "object" && !Array.isArray(sq) ? (sq as Record<string, number>) : undefined,
...
export async function fetchOrderForSeller(orderId: string, sellerId: string): Promise<BuyerOrder | null> {
  ...
  try {
    ...
    if (userSnap.exists()) {
      ...
      const name = d.storeName || `${d.name || ""} ${d.lastname || ""}`.trim() || d.username || "Cliente"
...
/** Campos exigidos por firestore.rules + campos de visualización (títulos, tallas, código). */
...
export async function fetchOrdersForSeller(sellerId: string): Promise<BuyerOrder[]> {
  ...
  const uniqueBuyerIds = Array.from(new Set(rows.map((r) => r.buyerId).filter(Boolean)))
  ...
  await Promise.all(
    uniqueBuyerIds.map(async (buyerId) => {
      try {
        ...
        // perfil del comprador no legible — mostrar nombre genérico
      }
    }),
  )
